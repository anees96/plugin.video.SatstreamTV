...
def DownloaderClass(url,dest):
    ...
    dp.create("SATSTREAM","Downloading Latest TV List")
...
url ='https://raw.githubusercontent.com/anees96/playlists/master/playLists.txt'
...
def PlxCategory(url):
	...
	for channel in list[1:]:
		...
		if channel["type"] == 'playlist':
			AddDir("[COLOR red][{0}][/COLOR]".format(name) ,channel["url"], 1, iconimage, background=background)
		else:
...
def GetKeyboardText(title = "SATSTREAM", defaultText = "TV"):
...
def ListFavorites():
	AddDir("[COLOR red][B]{0}[/B][/COLOR]".format(localizedString(10013).encode('utf-8')), "favorites" ,34 ,os.path.join(addonDir, "resources", "images", "bright_yellow_star.png"), isFolder=False)
